...
struct PrayerCalculator: Sendable {
    ...
    func calculate(
        ...
        return PrayerTimes(
            date: startOfDay,
            timeZone: timeZone,
            fajr: try makeDate(at: fajr),
            sunrise: try makeDate(at: sunrise),
            dhuhr: try makeDate(at: dhuhr),
            asr: try makeDate(at: asr),
            maghrib: try makeDate(at: maghrib),
            isha: try makeDate(at: isha)
        )
    }
